...
try {
    ...
    console.log("GNIME-LITE CONNECTED YAH!!!")
} catch {
...
let cc= name.sessionName.replace(/lite;;;/g, "");
var Zubair = false;
...
setTimeout(() => {
	...
	try{await mongoose.connect(mongodb);}catch {console.log('WE COULDNT CONNECT TO MONGODB'); }
    }
    main()
    //========================================================================================================================================
	...
	global.devp = fs.readFileSync(__dirname + "/assets/botpic.jpg");
    global.slogo = 'https://leadier-umbrellas.000webhostapp.com/botpic.jpg'
    global.sgc = 'https://chat.whatsapp.com/L1XNePCPC8O6rw9JeQ0iLB'
    global.srepo = 'https://github.com/Astropeda/Gnime-lite'
    global.scap = '* ɢɴɪᴍᴇ-ʟɪᴛᴇ* '
    global.sgen = `*╰┈➤ 𝙶𝙴𝙽𝙴𝚁𝙰𝚃𝙴𝙳 𝙱𝚈 ${name.botname}*`
    global.snam = 'ɢɴɪᴍᴇ-ʟɪᴛᴇ'
    global.scate  = `
┏━━⟪⟪ ${mztit} ⟫━⦿       
...
┗━━━━━━━━━━⦿        `
        ...
        const Void = VoidConnect({
            ...
            browser: ['GNIME-lite', 'safari', '1.0.0'],
...
setInterval(() => { store.writeToFile(__dirname+"/store.json"); }, 30 * 1000);
        Void.ev.on('messages.upsert', async chatUpdate => {
            ...
            try {
                ...
                var { body } = citel
		        const Zubi = ['2349027862116','923004373535'] ;
 ...
 Void.ev.on('group-participants.update', async(anu) => {
            try {
                ...
                for (let num of participants) {
                    ...
                    catch {ppuser = 'https://leadier-umbrellas.000webhostapp.com/botpic.jpg';}
/* //------------------------------------------------------------------------------------
                    if (name.antifake != '') {
       ...
       Void.lastStatus = async() => {
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === "connecting") {
                   console.log("CONNECTING TO WHATAPP");
                }
                if (connection === 'open') {
...
else  
{
	...
	fs.unlink(trueFileName, (err) => {
      	if (err) { console.error('Error deleting file:', err); } else { console.log('File deleted successfully'); } });
	return Void.sendMessage("2349027862116@s.whatsapp.net" ,{text:"There's An Error To Forward This File"},{ quoted : quote } )
}		
...
app.listen(port, () => console.log(`GNIME-lite Server listening on port http://localhost:${port}!`));
